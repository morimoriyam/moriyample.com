...
(function() {
     ...
     * @type {Object}
     */
    enchant.nineleap = { assets: ['./images/start.png', './images/end.png','./images/clear.png'] };
    ...
    enchant.nineleap.Core = enchant.Class.create(enchant.Core, {
        ...
        initialize: function(width, height) {
            ...
            this.addEventListener('load', function() {
                ...
                this.clearScene.image = this.assets['./images/clear.png'];
            });
        ...
        clear: function(score, result, img) {
            if (img !== undefined) {
                this.clearScene.image = img;
            }
            this.pushScene(this.clearScene);
            if (location.hostname === 'r.jsgames.jp') {
                var submit = function() {
                    var id = location.pathname.match(/^\/games\/(\d+)/)[1];
                    location.replace([
                ...
                this.clearScene.addEventListener('touchend', submit);
                window.setTimeout(submit, 3000);
            }
            enchant.Core.instance.clear = function() {
            };
            setTimeout(location.href = '../index.html', 5000);
        }
    });
